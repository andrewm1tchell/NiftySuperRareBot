...
const tweet = require('./etienneCrauss_tweet');
const _ = require("lodash");

function pollOpenSea() {
    var lastSaleTime = cache.get('lastSaleTimeEtienneCrauss-OPENSEA', null);
    if (lastSaleTime === null || typeof(lastSaleTime) === 'undefined') {
        lastSaleTime = moment().add(3, 'h').unix();
        cache.set('lastSaleTimeEtienneCrauss-OPENSEA', lastSaleTime);
    }
    ...
    axios.get('https://api.opensea.io/api/v1/events', {
        ...
        params: {
            asset_contract_address: '0x48542625835a919059741b4441d737e0f9ae78a5',
        ...
        _.each(sortedEvents, (event) => {
            ...
            if(moment(created).unix() > lastSaleTime) {
                ...
                cache.set('lastSaleTimeEtienneCrauss-OPENSEA', moment(created).unix());
            }
        });
...
function pollObjkt() {
    var lastSaleTime = cache.get('lastSaleTimeEtienneCrauss-OBJKT', null);
    if (lastSaleTime === null || typeof(lastSaleTime) === 'undefined') {
        lastSaleTime = moment().add(3, 'h').unix();
        cache.set('lastSaleTimeEtienneCrauss-OBJKT', lastSaleTime);
    }
    axios({
  ...
  event(where: {price: {_gt: "0"},
   event_type: {_in: ["dutch_purchase", "accept_offer", "ask_purchase"]}, token: {creators: {creator_address: {_eq: "tz1g7Ntg9f4tnMAPvbYxcgLWew8iEesLc1oh"}}}}) {
    id
    amount
    event_type
    price
    ...
    timestamp
        ...
        _.each(svents, (event) => {
            ...
            if(unixC > lastSaleTime) {
                ...
                cache.set('lastSaleTimeEtienneCrauss-OBJKT', moment(created).unix());
            }
        });
