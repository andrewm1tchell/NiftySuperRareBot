...
setInterval(function () {
    if(!isPolling) {
        let currentItem = nifties[currentItem];
        if (currentItem.bot === "BEN") {
            pollSuperRareBen(currentItem.url, currentItem.imageUrl, currentItem.item, currentItem.useImage);
        } else if(currentItem.bot === "AZEKWOH") {
            pollNiftyGatewayAzekwoh(currentItem.url,currentItem.collectionId,currentItem.imageUrl,currentItem.name);
        } else {
            if (!currentItem.isSR) {
                pollNiftyGatewayKarisma(currentItem.url, currentItem + 1, currentItem.image,currentItem.name);
            } else {
                pollSuperrareKarisma(currentItem.url,currentItem+1,currentItem.image,currentItem.name);
            }
        }
    }
}, 500000);
